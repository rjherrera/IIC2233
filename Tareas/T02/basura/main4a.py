...
class Conexion:
    ...
    def __init__(self, id, origen=None, destino=None):
        ...
        self.numero_conexiones = 0
        self.__class__.conexiones.append(self)

    @property
    def random(self):
        return self.numero_conexiones > 2

    @property
    def alternante(self):
        return 1 < self.numero_conexiones <= 2
        ...
        while n_puertos <= actual_max:
            ...
            if pseudo_hash(actual, siguiente) not in self.conexiones:
                c = Conexion(id=siguiente_index,
                             ...
                             destino=siguiente)
                presente = False
                for conexion in puerto_actual.conexiones:
                    if conexion.id == c.id:
                        presente = True
                        conexion.numero_conexiones += 1
                if not presente:
        ...
        while n_iteraciones <= 3000000:
            ...
            if pseudo_hash(actual, siguiente) not in self.conexiones:
                c = Conexion(id=siguiente_index,
                             origen=actual,
                             destino=siguiente)
                puerto_actual = Puerto.get(actual)
                presente = False
                for conexion in puerto_actual.conexiones:
                    if conexion.id == c.id:
                        presente = True
